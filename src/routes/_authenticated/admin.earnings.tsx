    ...
    <Screen title="MzansiTalk Earnings">
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-2xl border border-border bg-card p-4">
          <p className="text-xs text-muted-foreground">Total Ad Revenue This Month</p>
      ...
      <section className="mt-4 rounded-2xl border border-border bg-card p-4">
        <h2 className="flex items-center gap-2 text-sm font-bold uppercase tracking-wide">
          <BarChart3 className="size-4 text-gold" /> 30 Day Earnings
        </h2>
        <div className="mt-4 flex h-40 items-end gap-1">
          {days.map((day) => (
            <div
              key={day.day}
              title={`${day.day}: R${day.total.toFixed(2)}`}
